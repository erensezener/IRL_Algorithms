...
import kr.ac.kaist.irl.fromTraj.TrajIrlProj;
...
/**
 * junit test case for IRL using the projection method.
 * 
 * @author Jaedeug Choi (jdchoi@ai.kaist.ac.kr)
 *
 */
public class TestTrajIrlProj extends TestCase {
	private long seed = 0;
	...
	private int nExps = 5;				// # of experiments
	private int nTrajs = 10;			// # of trajectories
	private int nSteps = 100;			// # of steps in each trajectory
	private int nIrlIters = 20;			// # of IRL iterations
	private int nBeliefs = 100;			// # of sampled beliefs for PBPI
	private double minDist = 1e-1;		// minimum distance between beliefs
...
//		probName = "milos-aaai97";
		probName = "Maze_4x3";
		basisType = "SA"; //"NC";
		 
		PomdpProblem pomdpProb = configure();		
		System.out.println("Test Trajectory Projection-IRL");
...
//		nSteps = (int) (Math.log(x) / Math.log(tmpPomdpProb.gamma));
	...
	public void repeatedExp() throws Exception {		
		...
		for (int t = 0; t < nExps; t++) {
			...
			TrajIrlProj irl = new TrajIrlProj(pomdpProb);
			...
			System.out.println("# " + irl.checkReward(minId));
