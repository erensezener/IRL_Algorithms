 ...
 *
 */
public class TrajIrlLp {
	final private double PENALTY_WEIGHT = 2;
	...
	private ArrayList<Vector> beliefs;
	...
	public TrajIrlLp(PomdpProblem _pomdpProb) {		
		pomdp = _pomdpProb;		
	...
	public void setParams(String fscFname, BasisFunctions _phi, int _nIters, 
		...
		if (nRepresentatives == 1) optFeatExp = mkTraj2();
		else {
			mkTrajs();
			optFeatExp = calFeatExp(trajs);
		}
//		mkTrajs();		
//		optFeatExp = calFeatExp(trajs);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for (int t = 1; t < nIters && diffV > eps; t++) {
			...
			if (bPrint) {
				...
				for (int i = 0; i < phiNum; i++)
					System.out.printf("[%2d]%.4f ", i, alpha.get(i));
				System.out.println();
			}
	...
	private void setObjFunc() throws Exception {
		...
		for (int pi = 0; pi < PI.size(); pi++) {
			for (int b = 0; b < beliefs.size(); b++) {
				exObj = cplex.sum(exObj, varMap.get(strEps(pi, b)));
			}
		}
		// -lambda*||R||_1
		for (int s = 0; s < nStates; s++) {
	...
	private void setConstVb(int pi) throws Exception {
		...
		for (int b = 0; b < beliefs.size(); b++) {
			Vector B = beliefs.get(b);
			for (int n = 0; n < nNodes; n++) {
				IloNumExpr exL = varMap.get(strVb(pi, b));
				IloNumExpr exR = cplex.numExpr();
				Iterator<VectorEntry> itB = Mtrx.Iter(B);
				while (itB.hasNext()) {
					VectorEntry veB = itB.next();
					int s = veB.index();
					IloNumVar varV = varMap.get(strV(pi, n, s));
...
/////////////////////////////////////////////////////////////////////////////////	
	...
	private Vector[] mkTraj2() throws Exception {
		...
		beliefs = new ArrayList<Vector>();
		beliefs.add(pomdp.start.copy());
...
//		diffV = Math.abs(trajExpV - optFsc.getV0());
		...
		results[0] = Mtrx.Vec(phiNum, useSparse);
		for (int i = 0; i < phiNum; i++) {
			double x = 0;
			for (int s = 0; s < nStates; s++)
				for (int a = 0; a < nActions; a++)
					x += coefs[s][a] * phi.get(i, s, a);
			if (x != 0) results[0].set(i, x / nTrajs);
		}
		...
		try {
			...
			diffV = Mtrx.calL2Dist(results[0], muOpt);
	...
	private void mkTrajs() {
		...
		beliefs.add(pomdp.start.copy());
		while (beliefs.size() < nRepresentatives && cntList.size() > 0) {
			int maxId = -1;
...
//		diffV = Math.abs(expValue - optFsc.getV0());
	...
	private Vector[] calFeatExp(ArrayList<int[]>[] trajList) {
		System.out.println("--- Generate feature expectation");
		double T0 = CpuTime.getCurTime();
		int numOfTraj = trajList.length;
		int numOfStep = trajList[0].size();
		int numOfBelief = beliefs.size();
		
		int[] cnt = new int[numOfBelief];		
		double[][][] coefs = new double[numOfBelief][nStates][nActions];
		for (int m = 0; m < numOfTraj; m++) {
			int[] t0 = new int[numOfBelief];
			for (int i = 0; i < numOfBelief; i++) t0[i] = -1;
			
			Vector b = pomdp.start.copy();
			for (int t = 0; t < numOfStep; t++) {
				int a = trajList[m].get(t)[0];
				int z = trajList[m].get(t)[1];
				
				for (int i = 0; i < numOfBelief; i++) {
					if (t0[i] == -1 && Mtrx.equal(b, beliefs.get(i))) {
						t0[i] = t;
						cnt[i]++;
					}
					if (t0[i] > -1) {
...
//		return diff;
		...
		for (int b = 0; b < beliefs.size(); b++) {
			...
			double V_pi = newFsc.calMaxV(beliefs.get(b));
