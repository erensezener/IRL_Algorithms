...
while carsplaced < sum(mdp_params.num_cars),
    ...
    if map1(x,l) == 0,
        if (carsinlane == 0 && carsb+carsf <= 1 && (carsff == 0 || carsf == 0) && (carsbb == 0 || carsb == 0)) ||...
            ...
            %fprintf(1,'Placed car %i of %i (%d)\n',carsplaced,sum(mdp_params.num_cars),carsinaround);
        end;
    end;
end;
    ...
    for lane=1:mdp_params.lanes,
        for spd=1:mdp_params.speeds,
            ...
            for action=1:5,
                ...
                %ax = mod(x + ns - 1 - 1, mdp_params.length) + 1;
...
[feature_data, r] = highwayfeatures(mdp_params, mdp_data);
true_feature_map  = [];
...
%             b0(s) = 1;
            isempty = true;
            for i = 1:2
                for l = 1:mdp_params.lanes
                    if map1(mod(x + i - 1, mdp_params.length) + 1, l) ~= 0
                        isempty = false;
                        break;
                    end
                end
